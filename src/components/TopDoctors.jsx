...
const TopDoctors = () => {
    ...
    const doctors = Array.isArray(context.doctors) ? context.doctors : [];
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10 '>
      <h1 className="text-3xl font-medium">Top Doctors To Book</h1>
      <p className="sm:w-1/3 text-center text-sm ">Simply Browse through or extensive list of trusted Doctors</p>
            ...
            <div onClick={()=>{navigate(`/appointment/${item._id}`);scrollTo(0,0)}} key={index} className="border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500">
                ...
                <div className="p-4 ">
                    <div className={`flex items-center gap-2 text-sm text-center  ${item.avaliable? 'text-green-500':'text-gray-500'} `}>
                        <p className={`w-2 h-2 ${item.avaliable? 'bg-green-500':'bg-gray-500'} rounded-full`}></p><p className="">{item.avaliable?'Available':'Not AVailable'}</p>
                    </div>
