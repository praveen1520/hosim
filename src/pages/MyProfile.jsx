...
const updateUserProfileData=async ()=>{
  ...
  return userData && (
    <div className="max-w-lg flex flex-col gap-2 text-sm">
      {
        isEdit ? <label htmlFor="image" className="">
          <div className="inline-block relative cursor-pointer">
            <img src={image ? URL.createObjectURL(image):userData.image} alt="" className="w-36 rounded opacity-75" />
            <img src={image ? '':assets.upload_icon} alt="" className="w-10 absolute bottom-12" />
          </div>
          <input onChange={(e)=>setImage(e.target.files[0])} type="file" className="" id="image" hidden/>
        </label> :  <img src={userData.image} alt="useriamge"  className='w-36 rounded '/>
      }
    ...
    <div className="">
      ...
      {isEdit?
      <p><input className="bg-gray-50" type="text" onChange={e=>setUserData(prev=>({...prev,address:{...prev.address,line1:e.target.value}}))} value={userData.address.line1} /><br />
      <input className="bg-gray-50" type="text" onChange={e=>setUserData(prev=>({...prev,address:{...prev.address,line2:e.target.value}}))} value={userData.address.line2} /></p>:<p className='text-gray-500'>{userData.address.line1}<br />{userData.address.line2}</p> }
    </div>
     </div>
        <div className="">
          <div className="">
